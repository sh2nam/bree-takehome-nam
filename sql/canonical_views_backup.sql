...
loan AS (
  SELECT
    ...
    SUM(CASE WHEN status='default' THEN 1 ELSE 0 END) AS loans_defaulted,
...
SELECT
  ...
  l.total_loans,
  l.avg_loan_amount,
  CASE WHEN l.total_loans > 0 THEN l.loans_repaid*1.0/l.total_loans END AS hist_repay_rate,
  CASE WHEN l.total_loans > 0 THEN l.loans_defaulted*1.0/l.total_loans END AS hist_default_rate,
  l.avg_late_days
...
SELECT
  ...
  CASE
    WHEN l.is_disbursed = 1
     AND l.is_default = 1
     AND date_diff('day', l.disbursed_at_utc, CAST(l.due_date_clean AS TIMESTAMP)) <= 30
    THEN 1 ELSE 0
  END AS default_30d
FROM v_fct_loans_clean l
...
SELECT
  ...
  -- Lifetime context
  urf.total_inflows               AS total_inflows_life,
  urf.total_outflows              AS total_outflows_life,
  urf.txn_count                   AS txn_count_life,
  urf.unique_spend_categories     AS unique_spend_categories_life,
  urf.txn_amount_volatility       AS txn_amount_volatility_life,
  urf.hist_repay_rate             AS hist_repay_rate,
  urf.hist_default_rate           AS hist_default_rate,
  urf.avg_late_days               AS avg_late_days_hist,
  urf.total_loans                 AS total_loans_hist,
